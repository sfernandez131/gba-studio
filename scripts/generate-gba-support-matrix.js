...
// Native functions whose call block the bridge drops (VM_CALL_NATIVE itself is
// bridged, so these would otherwise be invisible at the macro level).
const droppedNatives = [
  ...section(/const DROPPED_NATIVE_CALLS/).matchAll(
    /^\s+\["(_[a-z0-9_]+)"\]:\s*\{[^}]*?reason:\s*"([^"]+)"/gms,
  ),
...
const nativeTable = () => {
  if (droppedNatives.length === 0) return `_None._\n`;
  return (
    "`VM_CALL_NATIVE` is bridged, but these engine natives are not: the bridge " +
    "drops the whole block their GB Studio event emits (the pushes, the call and " +
    "the pop), logs it during eject, and fails the build if one is called in any " +
    "other shape.\n\n| Native | Why |\n|---|---|\n" +
    droppedNatives.map((n) => `| \`${n.name}\` | ${n.reason} |`).join("\n") +
    "\n"
  );
};
