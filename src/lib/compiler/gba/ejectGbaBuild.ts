...
const ejectGbaBuild = async ({
  ...
  // Music: each project track plays on one of three GBA audio paths.
  //  - .uge (M14d): gbavm's hUGE player, a port of GB Studio's own driver, on the 4 Game
  //    Boy PSG channels. The song is exported with the editor's own exportToC - the same C
  //    the GB build compiles - adapted for GCC by compileGbaHugeSong, one file per song
  //    (src/gba_song_<symbol>.c).
  //  - .mod, chosen per-track by settings.gbaAudioBackend: "dmg" (default) = the PSG via
  //    gbt-player (file -> dmg_audio/, bn::dmg_music_item), or "maxmod" = the GBA's native
  //    DirectSound tracker mixer (file -> audio/, bn::music_item).
  // One track-index space spans all three; gba_music_backend(idx) tells the engine which to
  // use so VM_MUSIC_PLAY resolves.
  ...
  const hugeExterns: string[] = [];
  const hugeCases: string[] = [];
  ...
  // Likewise generated .uge songs, so a removed track is not still compiled in.
  for (const f of await readdir(Path.join(engineRoot, "src"))) {
    if (/^gba_song_.*\.c$/.test(f)) {
      await remove(Path.join(engineRoot, "src", f));
    }
  }
  let musicIdx = 0;
  for (const track of projectData.music ?? []) {
    if (/\.uge$/i.test(track.filename)) {
      let c: string;
      try {
        const song = loadUGESong(
          await readFile(assetFilename(projectRoot, "music", track)),
        );
        if (!song) throw new Error("no song");
        c = compileGbaHugeSong(song, track.symbol);
      } catch (e) {
        warnings(`GBA: could not read music "${track.filename}"`);
        continue;
      }
      await writeFile(
        Path.join(engineRoot, "src", `gba_song_${track.symbol}.c`),
        c,
      );
      dataSymbols[`_${track.symbol}_Data`] = musicIdx;
      backendCases.push(`        case ${musicIdx}: return 2;`);
      hugeExterns.push(`extern "C" const hUGESong_t ${track.symbol}_Data;`);
      hugeCases.push(`        case ${musicIdx}: return &${track.symbol}_Data;`);
      progress(
        `Converting music ${track.filename} -> src/gba_song_${track.symbol}.c (hUGE)`,
      );
      musicIdx++;
      continue;
    }
    if (!/\.mod$/i.test(track.filename)) {
      warnings(
        `GBA: skipping music "${track.filename}" (only .uge and .mod tracks are supported)`,
      );
      continue;
    }
  ...
  const musicHeader = [
    "// Generated by GBA Studio - music track lookup (hUGE + DMG gbt-player + Maxmod DirectSound).",
  ...
  // Sound effects, on two paths:
  //  - .wav (M5b): emitted to gbavm's audio/ folder, where Butano's Maxmod backend makes a
  //    bn::sound_item on DirectSound, separate from the Game Boy channels; gba_sfx(idx).
  //  - everything else (M14f) - .vgm, FX Hammer, and the Play Tone / Beep / Crash events -
  //    is a gbvm sfx_player stream on the PSG. The GB compile has already produced those
  //    streams in compiledData.files (sounds/*.c); they are lifted into
  //    src/gba_psg_sfx.c and played by gbavm's port of the same player; gba_psg_sfx(idx).
  // Each sound symbol `_<sym>` maps to its index for VM_SFX_PLAY; a PSG effect's index
  // carries PSG_SFX_FLAG, and its `___mute_mask_<sym>` maps to the mask it borrows.
  const sfxIncludes: string[] = [];
  const sfxCases: string[] = [];
  await ensureDir(Path.join(engineRoot, "audio"));
  let sfxIdx = 0;
  const wavSymbols = new Set<string>();
  for (const sound of projectData.sounds ?? []) {
    if (sound.type !== "wav" || !/\.wav$/i.test(sound.filename)) {
      continue; // a PSG effect, below
    }
    wavSymbols.add(sound.symbol);
    try {
  ...
  const psgSfx = collectPsgSfx(compiledData.files, wavSymbols);
  const psgExterns: string[] = [];
  const psgCases: string[] = [];
  psgSfx.forEach(({ symbol, muteMask }, idx) => {
    dataSymbols[`_${symbol}`] = idx | PSG_SFX_FLAG;
    dataSymbols[`___mute_mask_${symbol}`] = muteMask;
    psgExterns.push(`extern "C" const uint8_t ${symbol}[];`);
    psgCases.push(`        case ${idx}: return ${symbol};`);
  });
  await writeFile(
    Path.join(engineRoot, "src", "gba_psg_sfx.c"),
    psgSfxC(psgSfx),
  );
  if (psgSfx.length > 0) {
    progress(
      `Converting ${psgSfx.length} PSG sound effect(s) -> src/gba_psg_sfx.c`,
    );
  }
  const sfxHeader = [
  ...
  // Tilesets (matrix slice B): Replace Tile events reference a tileset by its
  // `_<symbol>` data pointer. Same rule as emotes - the indices must exist
  // BEFORE the scripts are parsed, because parseGbvmAsm resolves the operand
  // through dataSymbols; registering later silently drops every Replace Tile.
  for (let t = 0; t < (projectData.tilesets ?? []).length; t++) {
    dataSymbols[`_${projectData.tilesets[t].symbol}`] = t;
  }
  ...
  // M8e: "Run Custom Code (C++)" events. Each becomes a gba_user_<idx>() snippet in
  // the generated gba_user_code.h; the op resolves the `_gba_user_<eventId>` symbol
  // (sanitized to match scriptBuilder.gbaCustomCode) via dataSymbols to the index.
  // Collected + dataSymbols set BEFORE the scripts are parsed (parseGbvmAsm).
  const userCodeSnippets: { idx: number; code: string }[] = [];
  const userCodeSeen = new Set<string>();
  walkScenesScripts(
    scenes,
    {
      customEvents: {
    ...
    (event) => {
      if (
        event.command === "EVENT_GBA_CUSTOM_CODE" &&
        typeof event.id === "string" &&
        !userCodeSeen.has(event.id)
      ) {
        userCodeSeen.add(event.id);
        const idx = userCodeSnippets.length;
        dataSymbols[`_gba_user_${event.id.replace(/[^a-zA-Z0-9]/g, "_")}`] =
          idx;
        userCodeSnippets.push({
          idx,
  ...
  const userCodeHeader = [
    ...
    "// (the VM_USER_CODE op operand). Included by hw.cpp after vm.h + Butano, so a",
    "// snippet has GBA_VAR(idx) (script variables), bn::* and hw_* in scope.",
    "#ifndef GBA_USER_CODE_H",
    "#define GBA_USER_CODE_H",
    "#define GBA_VAR(idx) (script_memory[idx])",
    ...userCodeSnippets.map((s) => `void gba_user_${s.idx}()\n{\n${s.code}\n}`),
    "inline void gba_user_run(int idx)",
    "{",
    "    switch(idx)",
    "    {",
    ...userCodeSnippets.map(
      (s) => `        case ${s.idx}: gba_user_${s.idx}(); break;`,
    ),
    "    default: break;",
    "    }",
    "}",
  ...
  // --- Tilesets: graphics/tileset_<i>.bmp + src/gba_tileset_assets.h ---------
  // Replace Tile events copy one tile's PIXEL DATA out of a project tileset over
  // whatever tile a map position currently shows (gbavm op 0x9C). The engine wants
  // raw bn::tile data, so each tileset is emitted through Butano's own
  // `regular_bg_tiles` pipeline rather than hand-packed here - grit already knows
  // how to pack 4bpp tiles, and reusing it keeps the format correct for free.
  // Same 4-shade mapping the mono background path uses (index 0 stays the GBA's
  // transparent colour), so a replaced tile lands in the palette the background
  // it overwrites is already drawn with. Colour projects use banked bg palettes;
  // matching those per-tileset is follow-up work, noted in the design doc.
  const tilesetPalette: Rgb[] = [
    hexToRgb(settings.customColorsBlack || "202850"),
    hexToRgb(settings.customColorsWhite || "E8F8E0"),
    hexToRgb(settings.customColorsLight || "B0F088"),
    hexToRgb(settings.customColorsDark || "509878"),
  ...
  const tilesetIncludes: string[] = [];
  const tilesetCases: string[] = [];
  for (let t = 0; t < (projectData.tilesets ?? []).length; t++) {
    const tileset = projectData.tilesets[t];
    let img: { width: number; height: number; data: Uint8Array };
    try {
      img = await readFileToIndexedImage(
        assetFilename(projectRoot, "tilesets", tileset),
        tileDataIndexFn,
      );
    } catch (err) {
      warnings(`GBA: could not read tileset "${tileset.filename}"`);
      continue;
    }
    const name = `tileset_${t}`;
    await writeFile(
      Path.join(engineRoot, "graphics", `${name}.bmp`),
      indexedImageToBmp(img, tilesetPalette, { align: 8 }),
    );
    await writeFile(
      Path.join(engineRoot, "graphics", `${name}.json`),
      JSON.stringify(
        // 4bpp: the same depth the mono background path uses, so a replaced
        // tile matches the background it is drawn over.
        { type: "regular_bg_tiles", ["bpp_mode"]: "bpp_4" },
        null,
        2,
      ) + "\n",
    );
    tilesetIncludes.push(`#include "bn_regular_bg_tiles_items_${name}.h"`);
    tilesetCases.push(
      `    case ${t}: { const bn::span<const bn::tile>& s = ` +
        `bn::regular_bg_tiles_items::${name}.tiles_ref(); ` +
        `if (len) *len = int(s.size()); return s.data(); }`,
    );
    progress(`Converting tileset ${tileset.filename} -> ${name}`);
  }
  const tilesetHeader = [
  ...
  await writeFile(
    Path.join(engineRoot, "src", "gba_tileset_assets.h"),
    tilesetHeader + "\n",
  );
