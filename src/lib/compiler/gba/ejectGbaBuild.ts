...
const ejectGbaBuild = async ({
  ...
  // Music (M5a): emit each project DMG track to dmg_audio/ so Butano's DMG audio backend
  // (= gbt-player, the same engine as GB Studio's "gbt" driver) compiles it, and map its
  // data symbol (`_<sym>_Data`) to a track index so VM_MUSIC_PLAY resolves. Only .mod
  // tracks for now (Butano DMG also takes .vgm/.s3m; .uge needs a .vgm export - skipped).
  const dataSymbols: Record<string, number> = {};
  const musicIncludes: string[] = [];
  const musicCases: string[] = [];
  await ensureDir(Path.join(gbaEngineRoot, "dmg_audio"));
  let musicIdx = 0;
  for (const track of projectData.music ?? []) {
    if (!/\.mod$/i.test(track.filename)) {
      warnings(
        `GBA: skipping music "${track.filename}" (only .mod DMG tracks supported so far)`,
      );
      continue;
    }
    try {
      ...
      await writeFile(
        Path.join(gbaEngineRoot, "dmg_audio", `${track.symbol}.mod`),
        data,
      );
    } catch (e) {
    ...
    musicIncludes.push(`#include "bn_dmg_music_items_${track.symbol}.h"`);
    musicCases.push(
      `        case ${musicIdx}: return &bn::dmg_music_items::${track.symbol};`,
    );
    progress(`Converting music ${track.filename} -> dmg_audio/${track.symbol}.mod`);
    musicIdx++;
  }
  const musicHeader = [
