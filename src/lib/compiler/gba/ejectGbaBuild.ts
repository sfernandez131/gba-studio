...
import { writeFont } from "./writeFont";

// The subset of PrecompiledFontData the GBA font pipeline needs (id+symbol +
// the compiled glyph data). Kept structural to avoid a hard type dependency.
type GbaFont = {
  id: string;
  symbol: string;
  table: number[];
...
const ejectGbaBuild = async ({
  ...
  // Fonts are referenced by IMPORT_FAR_PTR_DATA _<fontSymbol>; on GBA, font
  // selection rides VM_SET_FONT by integer index (the order of usedFonts, which
  // is the order _getFontIndex assigns). This lets the bridge recognise font
  // far-ptrs instead of warning about them.
  const usedFonts = compiledData.usedFonts ?? [];
  const fontSymbolToIndex: Record<string, number> = {};
  usedFonts.forEach((font, i) => {
    fontSymbolToIndex[`_${font.symbol}`] = i;
  });
  ...
  // --- Fonts (P3): GB Studio fonts -> Butano sprite_fonts + font_table ---------
  // Iterate usedFonts in order (== the VM_SET_FONT / \002 font index).
  const fontTextColor = settings.customColorsWhite || "E8F8E0";
  const fontConversions = [];
  for (const font of usedFonts) {
    progress(`Converting font ${font.symbol}...`);
    fontConversions.push(
      await writeFont(font, font.symbol, gbaEngineRoot, fontTextColor),
    );
  }
  const defaultFontId = (settings as { defaultFontId?: string }).defaultFontId;
  const defaultFontIndex = Math.max(
    0,
    usedFonts.findIndex((f) => f.id === defaultFontId),
  );
  await writeFontTable(gbaEngineRoot, fontConversions, defaultFontIndex);
...
// --- font_table.h / font_table.cpp codegen (P3) -----------------------------
const writeFontTable = async (
  root: string,
  fonts: { name: string; include: string; widths: number[] }[],
  defaultIndex: number,
) => {
  const header = [
  ...
  await writeFile(Path.join(root, "src", "font_table.h"), header);

  const lines: string[] = [
    "// Generated by GBA Studio - font registry data.",
    '#include "font_table.h"',
    '#include "bn_utf8_characters_map_ref.h"',
    ...fonts.map((f) => f.include),
    "",
  ];
  // Each font: a 95-entry width array ([space, '!'..'~']) + a constexpr sprite_font
  // (ASCII-only, so an empty utf8 map). gba_fonts[] indexes by usedFonts order.
  fonts.forEach((f, i) => {
    lines.push(
      `static constexpr signed char gbafont_${i}_widths[] = { ${f.widths.join(", ")} };`,
    );
    lines.push(
      `static constexpr bn::sprite_font gbafont_${i}(bn::sprite_items::${f.name}, ` +
        `bn::utf8_characters_map_ref(), gbafont_${i}_widths);`,
    );
  });
  if (fonts.length > 0) {
    lines.push("const bn::sprite_font* const gba_fonts[] = {");
    fonts.forEach((_, i) => lines.push(`    &gbafont_${i},`));
    lines.push("};");
  } else {
    // C++ forbids zero-size arrays; emit a dummy (count 0 means it is never read).
    lines.push("const bn::sprite_font* const gba_fonts[] = { nullptr };");
  }
  lines.push(`const unsigned int gba_font_count = ${fonts.length};`);
  lines.push(`const unsigned int gba_default_font_index = ${defaultIndex};`);
  lines.push("");
  await writeFile(Path.join(root, "src", "font_table.cpp"), lines.join("\n"));
};
