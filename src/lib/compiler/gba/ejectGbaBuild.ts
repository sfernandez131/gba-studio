...
const ejectGbaBuild = async ({
  ...
  // --- Dialogue font: graphics/dialogue_font.bmp + src/gba_font_assets.h (M4n) --
  // Convert the project's default font (a grid of 8x8 glyphs, GB Studio chars from
  // ASCII 32, 16 per row) to a Butano sprite font, so dialogue renders in the real
  // game font instead of Butano's stand-in. First version: the default font,
  // fixed-width (e.g. gbs-mono). Variable-width + font switching are follow-ups.
  ...
  if (defaultFont) {
    try {
      ...
      for (let i = 0; i < FONT_GLYPHS; i++) {
        const gi = i + 1; // image glyph index (0 = space, 1 = '!', ...)
        const col = gi % cols;
        const row = Math.floor(gi / cols);
        for (let y = 0; y < FONT_GH; y++) {
          for (let x = 0; x < 8; x++) {
            strip[(i * FONT_GH + y) * 8 + x] =
              fontImg.data[(row * FONT_GH + y) * fontImg.width + (col * 8 + x)] ?? 0;
          }
        }
      }
