...
export interface GbaSceneEntry {
  ...
  // The scene's combined player-hit script C name (M10g), run when a projectile
  // hits the player; branches on the projectile's group. "0" when the scene
  // has no player On Hit tabs.
  playerHit: string;
}
...
export function formatGbaScenesC(
  ...
  for (const s of scenes) {
    ...
    if (s.playerHit !== "0") externs.add(s.playerHit);
  }
  const out: string[] = [
