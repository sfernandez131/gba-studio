...
export function linkGbaProgram(procs: GbaProc[]): LinkResult {
  ...
  for (const p of procs) {
    ...
    for (const sr of program.symRelocs) {
      ...
      // kind "code": another script proc, else a native fn / far data (M1c).
  ...
  return {
    ...
    source: formatLinkedC(linked, engineVarList),
  };
}

/** Emit the combined C: engine vars, every proc's bytecode/relocs + symbolic relocs, and the manifest. */
export function formatLinkedC(linked: LinkedProc[], engineVars: string[]): string {
  const out: string[] = [
    ...
    "// proc) and symbolic relocations (&target: another proc's array, or &engineVar);",
    "// see gba_link.h.",
    '#include "gba_link.h"',
    "",
  ];
