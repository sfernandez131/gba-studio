...
export const GBA_OPCODE_SPECS: Record<number, GbaOperandType[]> = {
  ...
  0x6d: [], // RANDOMIZE: reseed the RNG from a fast counter + the frame count (gbs2 G1)
...
export function formatGbaProgramC(name: string, program: GbaProgram): string {
  ...
  return [
    ...
    "// EWRAM (256 KB), not .data's default IWRAM (32 KB, shared with the engine), which",
    "// a real project's scripts overflow (gbs2 does). crt0 copies .ewram in at boot.",
    `__attribute__((section(".ewram"))) unsigned char ${name}[] = {`,
    ...rows,
    "};",
